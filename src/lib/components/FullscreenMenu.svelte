...
<dialog id="fullscreen_menu" class="modal">
	<div
	...
	>
		...
		<div class="flex items-center justify-between border-b border-b-neutral-200 py-3">
			<h3 class="text-primary my-0 text-lg font-bold">
				<a href="/" class="btn btn-ghost text-2xl"> Southern MN TCT</a>
			</h3>
			<!-- Close btn -->
			<form method="dialog">
		...
		<div class="overflow-y-auto">
			<ul class="menu px-0">
				{#each navData as { path, name, submenu }}
					<li class="px-0">
						<a href={path} class="btn btn-ghost justify-start text-2xl font-normal">{name}</a>
						{#if submenu}
							<ul class="menu my-0">
								{#each submenu as { name, path }}
									<li>
										<a href={path} class="btn btn-ghost justify-start text-xl font-normal">{name}</a
										>
									</li>
								{/each}
							</ul>
